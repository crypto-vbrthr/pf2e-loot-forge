...
export class StatueGenerator extends ProceduralGenerator {
  ...
  build(data, themeId, environment) {
    const template = this.pickTemplate(data), quality = this.pick(data.qualities, themeId), material = this.pick(data.materials, themeId), subject = this.pick(data.subjects, themeId), condition = this.pick(this.conditionPool(data, environment), themeId), flair = this.pick(data.flairs ?? [], themeId);
