...
export class TextileGenerator extends ProceduralGenerator {
  ...
  build(data, themeId, environment) {
    const template = this.pickTemplate(data), quality = this.pick(data.qualities, themeId), kind = this.pick(data.kinds, themeId), subject = this.pick(data.subjects, themeId), material = this.pick(data.materials, themeId), condition = this.pick(this.conditionPool(data, environment), themeId), flair = this.pick(data.flairs ?? [], themeId);
